...
// Version is the build version of utnm, overwritten during build with metadata.
var Version = "dev"
...
func main() {
	...
	switch flag.Arg(0) {
	case "install":
		kbnmPath, err := findUntraceableBinary(utnmBinary)
		if err != nil {
			exit(2, "error finding utnm binary: %s", err)
		}
		log.Print("installing: ", kbnmPath)
		if err := installer.InstallUTNM(kbnmPath); err != nil {
			exit(2, "error installing utnm whitelist: %s", err)
		}
		exit(0, "Installed NativeMessaging whitelists.")
	case "uninstall":
		if err := installer.UninstallUTNM(); err != nil {
			exit(2, "error uninstalling utnm whitelist: %s", err)
		}
