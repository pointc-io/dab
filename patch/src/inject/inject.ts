...
let KEY = 'P';
let MAGIC = 'AAAAAAAAAAAAAAAAAAAAAAAA';

function inject(actualCode: string) {
...
// Synchronous local storage getItem.
// let profile = localStorage.getItem(KEY);
if (!profile) {
    // Abort page load.
    window.stop();
    // Ask background for profile.
    port.postMessage({type: 'NEED_PROFILE'});
} else {
        ...
        function patchNavigator(protoDef: ProtoDef) {
            class Builder extends PropertyBuilder {
                ...
                getter(name: string, descriptor: PropertyDescriptor): PropertyInterceptor | any {
                    ...
                    switch (name) {
                        case 'userAgent':
                            return new UserAgentGetter(() => device && device.navigator && device.navigator.userAgent ? device.navigator.userAgent : "NOPE");
        ...
        if (isFirefox || isChrome || isOpera) {
            ...
            function findIFrames(list: Array<HTMLIFrameElement>, node: Node) {
                if (node.nodeName === 'IFRAME') {
                    ...
                    // Object.defineProperty(iframe.contentWindow.navigator, 'appCodeName', {get: function() {return "no";}});
                    // Object.defineProperty(iframe.contentWindow.navigator, 'appCode', {value: 'no'});
                    patchIFrame(iframe);
                    list.push(iframe);
                } else {
...
//                         // iframe.setAttribute('sandbox', 'allow-scripts');
//
//                         // Object.defineProperty(win.navigator, 'userAgent', {value: 'no'});
//                         // Object.defineProperty(win.navigator, 'appCodeName', {get: function() {return "no";}});
//                         // Object.defineProperty(win.navigator, 'appCode', {value: 'no'});
//                         // console.log(win.navigator.appCodeName);
//
//                         let script = document.createElement('script');
//                         script.textContent = actualCode;
//                         (document.head || document.documentElement).appendChild(script);
